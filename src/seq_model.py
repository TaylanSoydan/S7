...
class StackedEncoderModel(nn.Module):
    ...
    :param num_layers_per_stage: the number of SSM layers to stack
    ...
    def setup(self):
        """
        Initializes a linear encoder and the stack of SSM layers.
        """
...
def timepool(x, integration_timesteps):
    ...
    """
    T = jnp.sum(integration_timesteps, axis=0)
    integral = jnp.sum(x * integration_timesteps[..., None], axis=0)
    return integral / T
...
def masked_timepool(x, lengths, integration_timesteps, eps=1e-6):
    ...
    L = x.shape[0]
    mask = jnp.arange(L) < lengths
    T = jnp.sum(integration_timesteps)
...
class ClassificationModel(nn.Module):
    """
    S7 classificaton sequence model. This consists of the stacked encoder
    ...
    def setup(self):
        """
        Initializes the stacked SSM encoder and a linear decoder.
        """
...
class RetrievalModel(nn.Module):
    ...
    def __call__(self, x, integration_timesteps, length, train):
        ...
        if self.classification_mode in ["pool"]:
            ...
            raise NotImplementedError("Mode must be in ['pool', 'last]")
        x0, x1 = jnp.split(x, 2)
        features = jnp.concatenate([x0, x1, x0-x1, x0*x1], axis=-1)
