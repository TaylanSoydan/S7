...
from .ssm_init import (
    make_DPLR_HiPPO, init_VinvB, init_CV,
    trunc_standard_normal, init_log_steps,
)
...
def discretize_async(Lambda, step_delta, time_delta):
    ...
    :return: discretized Lambda_bar (complex64), B_bar (complex64) (P,), (P,H)
    """
    Identity = jnp.ones_like(Lambda)
    Lambda_bar = jnp.exp(Lambda * step_delta * time_delta)
    gamma_bar = (1/Lambda * (jnp.exp(Lambda * step_delta)-Identity))
...
class S7SSM(nn.Module):
    ...
    def setup(self):
        ...
        if self.log_a:
            ...
            Lambda = jnp.sqrt(( -1/Lambda - self.b ) / self.a)
    ...
    def __call__(self, input_sequence, integration_timesteps):
        ...
        """
        if self.log_a:
            Lambda = -jnp.exp(self.Lambda)
        elif self.stablessm_a:
            Lambda = -jnp.sqrt((-1 - self.b * self.Lambda)/(self.a*self.Lambda))
        else:
            Lambda = self.Lambda
        ...
        if self.input_dependent:
            if self.separate_dbc:
                ...
                step, B_projected, C_projected = jnp.split(x_dbl, indices_or_sections = [self.dt_rank,2*self.H_in*self.local_P+self.dt_rank],axis = -1)
