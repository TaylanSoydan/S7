...
class SequenceLayer(nn.Module):
    """
    Defines a single SSM layer, with nonlinearity,
...
class SequenceStage(nn.Module):
    """
    Defines a block of SSM layers with the same hidden size and event-resolution.
    """
    ssm: nn.Module
    discretization: str
    d_model_in: int
    ...
    def __call__(self, x, integration_timesteps, train: bool = True):
        ...
        x, integration_timesteps = SSMLayer(
