...
class Rotate:
    ...
    def __call__(self, events):
        if np.random.rand() > self.p:
            return events
        #print("initial max x = ",np.max(events["x"]))
        #print("initial min x = ",np.min(events["x"]))
        #print("initial max y = ",np.max(events["y"]))
        #print("initial min y = ",np.min(events["y"]))
        ...
        events['x'] = np.clip(events['x'], 0, self.sensor_size[0] - 1) # added -1
        events['y'] = np.clip(events['y'], 0, self.sensor_size[1] - 1) # added -1

        #print("augmented max x = ",np.max(events["x"]))
        #print("augmented min x = ",np.min(events["x"]))
        #print("augmented max y = ",np.max(events["y"]))
        #print("augmented min y = ",np.min(events["y"]))
