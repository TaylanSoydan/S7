...
def create_lra_listops_classification_dataset(cache_dir: Union[str, Path] = DEFAULT_CACHE_DIR_ROOT,
	...
	dir_name = '/data/storage/tsoydan/data/long-range-arena/lra_release/lra_release/listops-1000'
...
def create_lra_path32_classification_dataset(cache_dir: Union[str, Path] = DEFAULT_CACHE_DIR_ROOT,
	...
	dir_name = f"/data/storage/tsoydan/data/long-range-arena/lra_release/lra_release/pathfinder{resolution}"
	...
	tokens = next(iter(val_loader))
	print(tokens[0][0][0:50])
	
	#print(torch.unique(data[0]))	
	#assert 1 == 2
