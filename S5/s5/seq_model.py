...
class MixStackedEncoderModel(nn.Module):
    """ Defines a stack of S5 layers to be used as an encoder.
        Args:
            ssm         (nn.Module): the SSM to be used (i.e. S5 ssm)
            d_model     (int32):    this is the feature size of the layer inputs and outputs
    ...
    def setup(self):
        ...
        Initializes a linear encoder and the stack of S5 layers.
        """
        print("setup mixture model")
        self.encoder = nn.Dense(self.d_model)
        #self.layers = [
        #    SequenceLayer(ssm=self.ssm,dropout=self.dropout,d_model=self.d_model,activation=self.activation,training=self.training,prenorm=self.prenorm,batchnorm=self.batchnorm,bn_momentum=self.bn_momentum,step_rescale=self.step_rescale,)
        #    for _ in range(self.n_layers)
        #]
        self.layers = [
            SequenceLayer(
                ssm=(self.s5ssm if i % 2 == 0 else self.s7ssm),
                dropout=self.dropout,
                d_model=self.d_model,
                activation=self.activation,
                training=self.training,
...
class MixClassificationModel(nn.Module):
    ...
    def setup(self):
        ...
        Initializes the S5 stacked encoder and a linear decoder.
        """
        self.encoder = MixStackedEncoderModel(
                            s5ssm=self.s5ssm,
                            s7ssm=self.s7ssm,
                            d_model=self.d_model,
                            n_layers=self.n_layers,
...
# Here we call vmap to parallelize across a batch of input sequences
MixBatchClassificationModel = nn.vmap(
