...
def create_train_state(model_cls,
    ...
    if batchnorm:
        # Check if variables["params"] is a FrozenDict and unfreeze if necessary
        params = variables["params"]
        if isinstance(params, FrozenDict):
            params = params.unfreeze()
        
        # Assuming batch_stats is used as is
    ...
    # Proceed with Optax or other operations on params
...
def train_step(state,
    ...
    #for param_path, grad in jax.tree_flatten(grads)[0]:
    #    if 'proj' in param_path:
    #        print(f"Grad for {param_path}: {grad}")
    #        assert jax.numpy.all(grad == 0), f"Gradients for {param_path} are not frozen!"
